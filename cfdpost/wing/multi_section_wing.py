...
class MultiSecWing(BasicParaWing):
    ...
    @classmethod
    def _reconstruct_surface_grids(cls, g: dict, nx: int, nzs: List[int], tail: float=0.008) -> np.ndarray:
        '''
        ...
        if 'control_points' not in g.keys():
            if 'DAs' in g.keys():
                _control_points = np.concatenate((np.linspace(cabin, kink, 3), np.linspace(kink, half_span, 5)[1:]), axis=0)
        ...
        if 'DA_kink' in g.keys():
            ...
            zLE_cs = CubicSpline(_control_points, [reduce(lambda x, y: x + y, g['DAs'][:i+1]) for i in range(len(g['DAs']))], extrapolate=False)
            zLEs = zLE_cs(yLEs)
        ...
        if 'rtcs' in g.keys():
            ...
            for i in range(len(_control_points)):
                xx1, yus, yls, _, _ = cst_foil(nn=nx, cst_u=g['cst_u'][i], cst_l=g['cst_l'][i], t=g['tmaxs'][i] if 'tmaxs' in g.keys() else None, tail=tail)
        ...
        # twist
        tws_cs = CubicSpline(_control_points_mid, [reduce(lambda x, y: x + y, g['twists'][:i+1]) for i in range(len(g['twists']))], extrapolate=True)
        tws = tws_cs(yLEs)
        ...
        block = np.stack((xxs1, np.repeat(yLEs[None, :], 2*nx-1, axis=0), zzs1)).transpose(2, 1, 0)[..., [0, 2, 1]]
        # block[..., 2] = -block[..., 2]
